...
double henyey_greenstein_cdf(double theta)
{
    // Calculate the CDF for the Henyey-Greenstein phase function
    // g is the asymmetry parameter
    double g = 0;
    double cdf = 0;
    double dtheta = 0.1;  // step size for the integration

    // Integrate the PDF from 0 to theta
    for (double t = 0; t < theta; t += dtheta) { //TODO funzione analitica?
        cdf += dtheta * (1 - g * g) / std::pow(1 + g * g - 2 * g * std::cos(t), 1.5);
    }
...
Vector generateRandomDirection(std::mt19937& rng, const Vector& incomingDirection) {
  ...
  double deflectionAngle = inverse_transform_sampling(henyey_greenstein_cdf, rng);//è il coseno
  std::uniform_real_distribution<double> uniform(0, 1);  // uniform distribution from 0 to 1
  ...
  v.z = cosTheta;
  */
  return computeOutputVersor(incomingDirection, acos(deflectionAngle), 2 * PI * u);
}
...
int main() {//eulero stocastico
  ...
  for( int j = 0; j<1; ++j){
    ...
    for (int i = 0; i < numSteps; ++i) {
      ...
      if (rand01(rng) < exp(-mu_s * stepSize)) {
      ...
      if (pos_z<0 && photon.position.z> 0){
